...
xcodebuild build \
    ...
    HEADER_SEARCH_PATHS="${FFMPEG_KIT}/prebuilt/apple-ios-arm64-simulator/ffmpeg/include ../../shared/thirdparty/" \
    LIBRARY_SEARCH_PATHS="${FFMPEG_KIT}/prebuilt/apple-ios-arm64-simulator/ffmpeg/lib" \
...
xcodebuild build \
    ...
    ARCHS=arm64 \
    -configuration "Release" \
    HEADER_SEARCH_PATHS="${FFMPEG_KIT}/prebuilt/apple-ios-arm64/ffmpeg/include ../../shared/thirdparty/" \
    LIBRARY_SEARCH_PATHS="${FFMPEG_KIT}/prebuilt/apple-ios-arm64/ffmpeg/lib" \
    OTHER_CFLAGS="-DDISABLE_LOGGING -fembed-bitcode" \
    OTHER_LDFLAGS="-framework CoreFoundation -framework VideoToolbox -framework CoreVideo -framework CoreMedia -lavcodec -lavdevice -lavfilter -lavformat -lavutil -lswscale -lswresample -llibz -libbz2"
echo "DEVICE BUILD COMPLETE\n"

mkdir -p ../VologramsPlayer/Runtime/Plugins/iOS/
...
lipo -create \
    ...
    -output "../VologramsPlayer/Runtime/Plugins/iOS/libvolplayer.a"
